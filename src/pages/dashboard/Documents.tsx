    ...
    <div>
      ...
      <div
        ...
        className={`flex flex-col items-center justify-center rounded-2xl border-2 border-dashed px-6 py-12 text-center transition ${
          dragOver ? 'border-brand bg-brand-50' : 'border-slate-300 bg-white'
        }`}
      >
      ...
      <h2 className="mb-3 mt-8 text-sm font-semibold text-slate-700">Your library ({docs.length})</h2>
      {docs.length === 0 ? (
          ...
          {docs.map((d) => (
            <div
              ...
              className={`flex items-center gap-3 rounded-xl border bg-white p-3.5 ${
                activeId === d.id ? 'border-brand ring-1 ring-brand' : 'border-slate-200'
              }`}
            >
