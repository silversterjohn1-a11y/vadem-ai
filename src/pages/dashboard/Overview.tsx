  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {tools.map((t) => (
          <Link key={t.to} to={t.to} className="card group transition hover:-translate-y-1 hover:border-brand-200 hover:shadow-md">
            <div className="mb-3 grid h-11 w-11 place-items-center rounded-xl bg-brand-50 text-brand transition group-hover:bg-brand group-hover:text-white">
      ...
      {docs.length === 0 && (
        <div className="mt-8 rounded-2xl border border-brand-200 bg-brand-50 p-6 text-center">
          <p className="font-semibold text-slate-900">Get started in 30 seconds</p>
          <p className="mt-1 text-sm text-slate-600">Upload your first lecture PDF and let VademAI do the rest.</p>
